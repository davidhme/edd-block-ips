...
class MaxMind_Db_Reader
{
	 ...
	 * @throws MaxMind_Db_Reader_InvalidDatabaseException
	...
	public function __construct($database)
	{
		...
		$metadataDecoder = new MaxMind_Db_Reader_Decoder($this->fileHandle, $start);
		list($metadataArray) = $metadataDecoder->decode($start);
		$this->metadata = new MaxMind_Db_Reader_Metadata($metadataArray);
		$this->decoder = new MaxMind_Db_Reader_Decoder(
			$this->fileHandle,
	...
	private function findAddressInTree($ipAddress)
	{
		...
		throw new MaxMind_Db_Reader_InvalidDatabaseException("Something bad happened");
	}
	...
	private function readNode($nodeNumber, $index)
	{
		...
		switch ($this->metadata->recordSize) {
			case 24:
				$bytes = MaxMind_Db_Reader_Util::read($this->fileHandle, $baseOffset + $index * 3, 3);
				list(, $node) = unpack('N', "\x00" . $bytes);
				return $node;
			case 28:
				$middleByte = MaxMind_Db_Reader_Util::read($this->fileHandle, $baseOffset + 3, 1);
				...
				$bytes = MaxMind_Db_Reader_Util::read($this->fileHandle, $baseOffset + $index * 4, 3);
				list(, $node) = unpack('N', chr($middle) . $bytes);
				return $node;
			case 32:
				$bytes = MaxMind_Db_Reader_Util::read($this->fileHandle, $baseOffset + $index * 4, 4);
				list(, $node) = unpack('N', $bytes);
				return $node;
			default:
				throw new MaxMind_Db_Reader_InvalidDatabaseException(
					'Unknown record size: '
	 ...
	 * @return MaxMind_Db_Reader_Metadata object for the database.
	 */
...
class MaxMind_Db_Reader_Decoder
{
	...
	public function decode($offset)
	{
		list(, $ctrlByte) = unpack(
			'C',
			MaxMind_Db_Reader_Util::read($this->fileStream, $offset, 1)
		);
		$offset++;
	...
	private function decodeByType($type, $offset, $size)
	{
		...
		$bytes = MaxMind_Db_Reader_Util::read($this->fileStream, $offset, $size);
		switch ($type) {
			case 'utf8_string':
	...
	private function decodePointer($ctrlByte, $offset)
	{
		...
		$buffer = MaxMind_Db_Reader_Util::read($this->fileStream, $offset, $pointerSize);
	...
	private function sizeFromCtrlByte($ctrlByte, $offset)
	{
		...
		$bytes = MaxMind_Db_Reader_Util::read($this->fileStream, $offset, $bytesToRead);
 ...
 */
class MaxMind_Db_Reader_InvalidDatabaseException extends Exception
{
}
 ...
 */
class MaxMind_Db_Reader_Metadata
{
